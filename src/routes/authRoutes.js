...
// Configuração do Supabase (acessa diretamente process.env)
...
if (!supabaseUrl || !supabaseAnonKey) {
    ...
    // Considere sair do processo ou lançar um erro mais robusto em produção
}
...
if (!SHOPEE_PARTNER_ID_LIVE || !SHOPEE_API_KEY_LIVE || !SHOPEE_AUTH_HOST_LIVE || !SHOPEE_API_HOST_LIVE) {
...
function generateShopeeSignature(path, params, timestamp) {
    // Para token_by_code ou access_token/get (como no seu caso)
    // A base string é partner_id + path + timestamp + JSON.stringify(body)
    // No seu código anterior para generateShopeeSignature, ele não usava o body.
    // Vamos ajustar para ser mais robusto, mas o principal é que a função é usada para diferentes endpoints.
    // Para getAccessTokenFromCode e refreshShopeeAccessToken, a assinatura precisa incluir o body.
    // Para endpoints GET como get_order_list, a assinatura pode ser só partner_id + path + timestamp + query_params_string.
    // Para simplicidade, vamos garantir que o `token_by_code` usa a assinatura correta no local da chamada.
    
    let baseString = `${SHOPEE_PARTNER_ID_LIVE}${path}${timestamp}`;
    
    // ATENÇÃO: A assinatura para token_by_code e access_token/get DEVE incluir o corpo da requisição!
    // Esta função genérica pode não ser adequada para todos os endpoints da Shopee.
    // Seu uso em getAccessTokenFromCode e refreshShopeeAccessToken precisa ser ajustado se o sign não está correto lá.
    // No seu código atual, `getAccessTokenFromCode` usa `generateShopeeSignature(path, {}, timestamp)`. Isso está INCORRETO para a Shopee.
    // A Shopee exige o JSON.stringify(requestBody) na assinatura para `token_by_code` e `access_token/get`.
    // Vamos corrigir isso diretamente nas funções.
...
async function getAccessTokenFromCode(code, shopId) {
    const path = "/api/v2/auth/token/get"; // Este é o endpoint correto para o seu caso (obtendo token pela primeira vez)
    ...
    // CORREÇÃO CRÍTICA: Assinatura para token/get DEVE incluir o JSON.stringify(requestBody)
    ...
    const url = `${SHOPEE_API_HOST_LIVE}${path}`; // URL base sem query params de sign
    
    try {
        const response = await axios.post(url, requestBody, {
            headers: { 
                'Content-Type': 'application/json',
                'Host': new URL(SHOPEE_API_HOST_LIVE).host, // Garante que o Host header é apenas o domínio
                'partner_id': SHOPEE_PARTNER_ID_LIVE,
                'timestamp': timestamp,
                'sign': sign // Assinatura vai no header/body, não na query para esta API
            }
        });
...
async function refreshShopeeAccessToken(shopId, refreshToken) {
    ...
    // CORREÇÃO CRÍTICA: Assinatura para access_token/get DEVE incluir o JSON.stringify(requestBody)
    const baseString = `${SHOPEE_PARTNER_ID_LIVE}${path}${timestamp}${JSON.stringify(requestBody)}`;
    const sign = crypto.createHmac('sha256', SHOPEE_API_KEY_LIVE).update(baseString).digest('hex');
...
// E agora também será o callback da Shopee
router.get('/', async (req, res) => { // Rota alterada para a raiz
    const { code, shop_id } = req.query;

    if (!code || !shop_id) {
        console.log('[API_ROUTE] Acesso à raiz sem code ou shop_id. Servidor rodando.');
        return res.status(200).send('Servidor BI Marketplace Integrator rodando! Se você está aqui após autorizar na Shopee, houve um erro no processamento do callback. Verifique os logs.');
    }
    ...
    try {
        ...
        // Validação adicional para shop_id e partner_id se necessário antes de upsert
        if (!shop_id || isNaN(Number(shop_id)) || !SHOPEE_PARTNER_ID_LIVE || isNaN(Number(SHOPEE_PARTNER_ID_LIVE))) {
        ...
        const { data, error: upsertError } = await supabase
            ...
            .upsert(
                {
                    shop_id: Number(shop_id), // Garante que é um número
                    access_token: tokens.access_token,
                    refresh_token: tokens.refresh_token,
                    token_expires_at: new Date(Date.now() + tokens.expire_in * 1000).toISOString(),
                    last_updated_at: new Date().toISOString(),
                    partner_id: Number(SHOPEE_PARTNER_ID_LIVE) // Garante que é um número
                },
...
router.get('/auth/shopee/fetch-orders', async (req, res) => {
    ...
    try {
        const { data: connectionData, error: fetchError } = await supabase
            .from('api_connections_shopee')
            .select('access_token, refresh_token, token_expires_at') // Corrigido para token_expires_at
            .eq('shop_id', shopId)
            .single();
        ...
        const expiresAt = new Date(connectionData.token_expires_at); // Corrigido para token_expires_at
        const now = new Date();
            ...
            try {
                ...
                const { error: updateError } = await supabase
                    ...
                    .upsert({
                        ...
                        refresh_token: refreshToken,
                        token_expires_at: newExpiresAt.toISOString() // Corrigido para token_expires_at
                    }, { onConflict: 'shop_id' });
        ...
        // Assinatura para get_order_list (GET) não precisa incluir o corpo, apenas a URL base string e os params
        ...
        const ordersUrl = `${SHOPEE_API_HOST_LIVE}${ordersPath}?partner_id=${SHOPEE_PARTNER_ID_LIVE}&timestamp=${timestamp}&sign=${signatureOrderList}&access_token=${accessToken}&shop_id=${shopId}&order_status=${orderParams.order_status}&page_size=${orderParams.page_size}`;
