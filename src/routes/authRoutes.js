...
router.get('/auth/shopee/fetch-orders', async (req, res) => {
    ...
    try {
        // Buscando da tabela client_connections com client_id e connection_name
        ...
        const now = new Date();
        const partnerIdFromDB = connectionData.additional_data ? connectionData.additional_data.partner_id : Number(SHOPEE_PARTNER_ID_LIVE);
        ...
        if (now >= expiresAt) {
            ...
            try {
                ...
                // Atualizando na tabela client_connections
        ...
        const timestamp = Math.floor(Date.now() / 1000);
        const partnerId = partnerIdFromDB; 

        // === Adicionando time_from e time_to ===
        ...
        const queryParams = {
            ...
            page_size: 10,
            time_from: timeFrom, // <<<<< Adicionado >>>>>
            time_to: timeTo,     // <<<<< Adicionado >>>>>
            time_range_field: 'create_time', // ou 'update_time', dependendo do que você quer filtrar
        };
        
        // Ordena os parâmetros para a assinatura (IMPORTANTE!)
        const sortedKeys = Object.keys(queryParams).sort();
        let baseStringOrderList = `${partnerId}${ordersPath}${timestamp}${accessToken}${Number(shopId)}`;
        // Adiciona time_from e time_to à base string para a assinatura
        baseStringOrderList += `${timeFrom}${timeTo}`; // <<<<< Adicionado na assinatura >>>>>
        ...
        const ordersUrl = `${SHOPEE_API_HOST_LIVE}${ordersPath}?${queryString}`;
